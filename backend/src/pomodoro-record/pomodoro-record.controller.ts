...
export class PomodoroRecordController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Pomodoro session saved',
    type: PomodoroRecord,
  })
  async saveRecord(
    @Body() createPomodoroRecordDto: CreatePomodoroRecordDto,
    @Request() req: any,
  ) {
  ...
  @ApiResponse({
    status: 200,
    description: 'User Pomodoro records retrieved',
    type: [PomodoroRecord],
  })
  ...
  @Get('/count-by-day')
  @ApiOperation({
    summary: 'Get Pomodoro session count per day within a date range',
  })
